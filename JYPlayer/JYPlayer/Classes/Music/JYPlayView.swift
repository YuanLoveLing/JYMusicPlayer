...
class JYPlayView: UIView {
    ...
    static let shareInstance = {
        return Bundle.main.loadNibNamed("JYPlayView", owner: nil, options: nil)?.first as! JYPlayView
    }()
    ...
    func showWithPlayerItem(music: JYMusic, isOnline: Bool) {
        ...
        UIView.animate(withDuration: 0.25, animations: {
            ...
            // 1、停止之前播放
            JYPlayerManager.shareInstance.destroy()
            // 2、开始现在播放
            self?.playerItem = JYPlayerManager.shareInstance.play(urlString: urlString, isOnline: isOnline)
    ...
    @IBAction fileprivate func playOrPauseButtonDidClick() {
        if JYPlayerManager.shareInstance.isPlaying == true {
            JYPlayerManager.shareInstance.pause()
            ...
            if let urlString = urlString {
                ...
                playerItem = JYPlayerManager.shareInstance.play(urlString: urlString, isOnline: false)
            }
        }
    }
